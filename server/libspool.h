 ...
 *
 * This is a modified version of the main Github repository, using
 * mmap() instead of malloc() due to child processes sharing memory
 */
 ...
 * calls to mmap() which slow execution
 */
pool *pool_init(int size){
  pool *p = mmap(NULL, sizeof(pool), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANON, -1, 0);
  p->ind = 0;
  p->size = size;
  p->avail = set_all(size);
  p->pool = mmap(NULL, sizeof(void*)*size, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANON, -1, 0);
...
/*
 * Returns whether a pool is
 * completely empty
 */
#define pool_empty(p) p->avail==set_all(p->size)
